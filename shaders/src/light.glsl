...
vec3 calculateAreaLight(Light light, vec3 N, vec3 V, vec3 P, vec3 albedo, float metallic, float roughness) {
    // Use light.data.xy as area size (width, height)
    ...
    float width = max(light.data.x, 0.0001);
    float height = max(light.data.y, 0.0001);
    float area = width * height;

    // Approximate area-light radiance:
    // scale by area, inverse-square falloff and by cosine between area normal and direction to P
    float areaNormalFactor = max(dot(normalize(light.direction.xyz), -L), 0.0);
    vec3 radiance = light.color.rgb * (area * areaNormalFactor / (distance * distance + 0.0001));
